...
public void evaluate( Chromosome c ) {
	...
	//TODO add in when desired to debug
	//logger.info("Evaluating: " +c);
	try {
		...
		for ( int i = 0; i < numTrials; i++ ) {
			...
			/*
			if (c.size() > 15) {
				XmlPersistableChromosome x = new XmlPersistableChromosome(c);
				System.out.println( x.toXml() );
				System.out.println("\n\n" + value);
				Scanner scan = new Scanner(System.in);
				System.out.print("Press any key when ready to continue: ");
				String s = scan.next();
			}
		...
		//TODO add in when desired to debug
		//logger.info("= Total Fitness =  " +c.getFitnessValue() );
		
		// print out when identified immediatley
...
//			networkInput[ 3 ] = 0.5; // bias
		...
		if (currentTimestep > 28 ) {
		
			//logger.info("networkOutput: " + networkOutput );
		...
		if ( ( state[ 0 ] < -trackLengthHalfed ) || ( state[ 0 ] > trackLengthHalfed )
			...
			//logger.info("Something made this a failure state!");
			//for (double d: state) {
				//System.out.print( d +" " );
			//}
			//logger.info( state[ 4 ] +" > " + poleAngleThreshold + " @ timestep " + currentTimestep );
			break;
		}
		
		//logger.info( state[ 4 ] +" < " + poleAngleThreshold );
