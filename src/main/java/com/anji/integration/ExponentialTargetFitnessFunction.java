 ...
 * such that max fitness is <code>Integer.MAX_VALUE</code>. See
 * <code>calculateErrorFitness()</code> and <code>calculateSkewedFitness()</code> for
 * details.
 * 
...
public class ExponentialTargetFitnessFunction extends TargetFitnessFunction {
	...
	public void init( Properties newProps ) {
		try {
			...
			for ( int i = 0; i < nullResponses.length; ++i )
				Arrays.fill( nullResponses[ i ], 0.0d );
	...
	protected int calculateErrorFitness( double[][] responses, double minResponse, double maxResponse ) {
		...
		double expFactor = ( Math.log( MAX_FITNESS ) + ( nullRawFitness - Math.E ) )
				/ maxRawFitnessValue;
