 ...
 * enumerated type constant for ANJI ANN
 */
public final static String GASNEAT_TYPE = "gasneat";
...
//TODO: test - this should not matter because it gets set in init()
private String type = GASNEAT_TYPE;

private int recurrentCycles;

private Properties props;

private AnjiNetTranscriber anjiTranscriber;
private GasNeatTranscriber gasNeatTranscriber;
...
public void init( Properties props ) {
	this.props = props;
	type = props.getProperty( TYPE_KEY, GASNEAT_TYPE );
	recurrentCycles = props.getIntProperty( RECURRENT_CYCLES_KEY, 1 );
	anjiTranscriber = (AnjiNetTranscriber) props.singletonObjectProperty( AnjiNetTranscriber.class );
	gasNeatTranscriber = (GasNeatTranscriber) props.singletonObjectProperty( GasNeatTranscriber.class );
}
...
public Activator newActivator( Chromosome ch ) throws TranscriberException {
	...
	if ( ANJI_TYPE.equals( type ) ) {
		result = new AnjiActivator( anjiTranscriber.newAnjiNet( ch ), recurrentCycles );
	}else if ( GASNEAT_TYPE.equals( type ) ) {
		result = new GasNeatActivator( gasNeatTranscriber.newGasNeatNet( ch, props ), recurrentCycles );
