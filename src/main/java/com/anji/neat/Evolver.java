...
public void init( Properties props ) throws Exception {
	...
	if ( props.get("ann.type" ).equals("anji") ) {
		...
		logger.info("Evolving an anji ANN.");
	} else if (  props.get("ann.type" ).equals("gasneat") ) {
		config = new GasNeatConfiguration( props );
		logger.info("Evolving an gasneat ANN.");
	} else {
		logger.error("ERROR - must specify either gasneat or anji in configuration file for property 'ann.type'");
	}
...
public void run() throws Exception {
	...
	for ( int generation = 0; ( generation < numEvolutions && adjustedFitness < targetFitness ); ++generation ) {
		...
		genotype.evolve(  generation, numEvolutions  );
	...
	logger.info("CHAMP: "+ champ.toString() );
	logger.info("ALLELES: "+ champ.getAlleles() );
	
	for (Object a: champ.getAlleles() ) {
		Allele allele = (Allele) a;
		//logger.info( "#"+allele.getInnovationId() + ": "+ allele.toString() );
		/*if ( a.getClass().equals( GasNeatNeuronAllele.class ) ) {
			GasNeatNeuronAllele g = (GasNeatNeuronAllele)a;
			logger.info( "#"+allele.getInnovationId() + ": "+ allele.toString() + g. );
		} else {
			logger.info( "#"+allele.getInnovationId() + ": "+ allele.toString() );			
		}
		//*/
	...
	XmlPersistableChromosome x = new XmlPersistableChromosome(champ);
	logger.debug( x.toXml() );
...
public static void main( String[] args ) throws Throwable {
	try {
		...
		System.out.println("This is the version in eclipse!");
