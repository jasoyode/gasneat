...
private float mutationStartingGeneration = 0.0f;
...
protected abstract void mutate( final Configuration config, final ChromosomeMaterial target,
		Set allelesToAdd, Set allelesToRemove, int currentGeneration, int maxGenerations ) throws InvalidConfigurationException;
 ...
 * @throws InvalidConfigurationException
 */
public void mutate( final Configuration config, final List offspring, int currentGeneration, int maxGenerations )
		throws InvalidConfigurationException {
	...
	while ( iter.hasNext() ) {
		...
		mutate( config, material, allelesToAdd, allelesToRemove, currentGeneration, maxGenerations );
...
protected boolean doesMutationOccur( int currentGeneration ) {
	return ( currentGeneration >=  mutationStartingGeneration );
}
...
protected static void updateMaterial( ChromosomeMaterial material, Set allelesToAdd,
	...
	/*
	System.out.println("************** ALLELES TO REMOVE ******************");
	for (Object o: allelesToRemove) {
		System.out.println(  ((Allele)o).getInnovationId()  );
	}
	System.out.println("***************************************************");
	...
	for (Object a: material.getAlleles() ) {
		System.out.println( a.toString() );
	}
	GasNeatChromosomeUtility.allelesHaveNothingMissing( material.getAlleles() );
	*/
 ...
 * @param aMutationRate
 */
protected void setMutationGeneration( float aGeneration ) {
	mutationStartingGeneration = aGeneration;
}
