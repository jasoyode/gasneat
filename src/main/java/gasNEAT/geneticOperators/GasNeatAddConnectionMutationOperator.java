 ...
 */
public class GasNeatAddConnectionMutationOperator extends AddConnectionMutationOperator {

	private static Logger logger = Logger.getLogger( GasNeatAddConnectionMutationOperator.class );
	
	/**
	 * properties key, add connection mutation rate
	 */
	public static final String ADD_CONN_MUTATE_RATE_KEY = "gasneat.add.connection.mutation.rate";
 ...
 */
public GasNeatAddConnectionMutationOperator() {
 ...
 */
public GasNeatAddConnectionMutationOperator( float newMutationRate ) {
 ...
 */
public GasNeatAddConnectionMutationOperator( RecurrencyPolicy aPolicy ) {
 ...
 * @see RecurrencyPolicy
 */
public GasNeatAddConnectionMutationOperator( float aMutationRate, RecurrencyPolicy aPolicy ) {
	super( aMutationRate );
	policy = aPolicy;
}
...
protected void mutate( Configuration jgapConfig, final ChromosomeMaterial target,
	...
	logger.debug( "MUTATION:   " + this.getClass().getCanonicalName() );
	...
	addConnections( numConnectionsToAdd, config, neuronList, conns, allelesToAdd, currentGeneration, maxGenerations );
}
...
private void addConnections( int numConnectionsToAdd, NeatConfiguration config,
		List neuronList, SortedMap conns, Set allelesToAdd, int currentGeneration, int maxGenerations ) {
...
private boolean connectionAllowed( NeuronAllele src, NeuronAllele dest, SortedMap conns ) {
	...
	boolean nonlinear = ActivationFunctionType.LINEAR.equals( dest.getActivationType() ) == false ;
	
	if (!nonlinear) {
		//System.out.println( "src "+src );
		//System.out.println( "dest "+dest );
		logger.debug("You probably do not want to allow a connection to a linear input!");
		logger.debug("GasNeatAddConnection.connectionAllowed");
		//System.exit(1);
	}
