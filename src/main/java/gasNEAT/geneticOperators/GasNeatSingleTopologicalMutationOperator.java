 ...
 * @author Philip
 */
public class GasNeatSingleTopologicalMutationOperator extends SingleTopologicalMutationOperator {

private final static float DEFAULT_MUTATION_RATE = calculateMutationRate(
		AddConnectionMutationOperator.DEFAULT_MUTATE_RATE,
...
private double addConnRatio = calculateAddConnRatio(
		AddConnectionMutationOperator.DEFAULT_MUTATE_RATE,
		AddNeuronMutationOperator.DEFAULT_MUTATE_RATE );
...
public void init( Properties props ) throws Exception {
	addConnOp = (AddConnectionMutationOperator) props
			.singletonObjectProperty( AddConnectionMutationOperator.class );
	addNeuronOp = (AddNeuronMutationOperator) props
...
private static double calculateAddConnRatio( float addConnMutationRate,
		float addNeuronMutationRate ) {
	return ( addConnMutationRate - ( addConnMutationRate * addNeuronMutationRate * 0.5 ) )
			/ ( addConnMutationRate + addNeuronMutationRate );
}

private static float calculateMutationRate( float addConnMutationRate,
		float addNeuronMutationRate ) {
	return addConnMutationRate + addNeuronMutationRate
			- ( addConnMutationRate * addNeuronMutationRate );
}
 ...
 */
public GasNeatSingleTopologicalMutationOperator() {
 ...
 * @param aPolicy
 */
public GasNeatSingleTopologicalMutationOperator( float addConnMutationRate,
		float addNeuronMutationRate, RecurrencyPolicy aPolicy ) {
 ...
 */
protected void mutate( Configuration jgapConfig, ChromosomeMaterial target, Set allelesToAdd,
		Set allelesToRemove, int currentGeneration, int maxGenerations ) {
	...
	if ( doesMutationOccur( rand ) ) {
		...
		if ( rand.nextDouble() < addConnRatio ) {
			...
			while ( iter.hasNext() && !isAdded ) {
				...
				isAdded = addNeuronOp.addNeuronAtConnection( config, NeatChromosomeUtility
						.getNeuronMap( alleles ), oldConnectAllele, allelesToAdd, allelesToRemove );
			}
		}
	}
}
