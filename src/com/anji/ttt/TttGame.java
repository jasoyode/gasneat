 ...
 * @author Derek James
 */
public class TttGame implements Game, Configurable {

private static Logger logger = Logger.getLogger( TttGame.class );
...
public GameResults play( PlayerResults contestantResults, PlayerResults opponentResults ) {
	...
	String XML_GAME_TAG = "Ttt";
	String XML_TYPE_TAG = "TttGame";
	...
	gameOneXml.append( board.getBoardSize() );
	gameOneXml.append( "</" ).append( XML_BOARDSIZE_TAG ).append( ">\n" );
	gameOneXml.append( "</" ).append( XML_INFORMATION_TAG ).append( ">\n" );
	...
	while ( gameOver == false ) {
		...
		moveColumn = ( newMove % board.getBoardSize() );
		switch ( moveColumn ) {
			case 0:
				MOVE_COLUMN = "A";
				break;
		...
		MOVE_ROW = ( ( newMove / board.getBoardSize() ) + 1 );
