...
protected ChromosomeMaterial reproduce( Configuration config, Chromosome dominantChrom,	Chromosome recessiveChrom,  int currentGeneration, int maxGenerations ) {
	...
	while ( iter.hasNext() ) {
		...
		if ( allele instanceof ConnectionAllele ) {
			...
			//System.out.println("all: " + allele   );
			//System.out.println("dom: " + dominantConnectionAllele   );
			//System.out.println("rec: " + recessiveConnectionAllele   );
			...
			if ( recessiveConnectionAllele != null ) {
				...
				if ( config.getRandomGenerator().nextBoolean() ) {
					...
					//System.out.println( dominantConnectionAllele   );
					//System.exit(1);
				}
			}
		}
