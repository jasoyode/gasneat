...
public final static String RECEPTOR_MAP_FILE_LOCATION_KEY = "receptor.map.file.location";
...
public NeatConfiguration( Properties newProps, boolean init ) throws InvalidConfigurationException {
	super();
	if (init) {
		init( newProps );
	}
...
private void init( Properties newProps ) throws InvalidConfigurationException {
	...
	if (this.getClass().equals(GasNeatConfiguration.class )) {
		//addReproductionOperator( cloneOper );
		//cloneOper.setSlice( survivalRate );
		System.out.println("DOES THIS REALLY HAPPEN???");
		System.exit(1);
	} else {
...
/**
 * @return maximum connection weight
 */
...
/**
 * @return minimum connection weight
 */
