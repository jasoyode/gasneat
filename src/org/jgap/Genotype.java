...
//jasonayoder - added current generation number for use with enabling mutations
//at different stages in evolution

public synchronized void evolve( int currentGeneration, int maxGenerations) {
	try {
		...
		while ( iterator.hasNext() ) {
			...
			logger.debug("repro OPERATOR: "+  operator     );
			logger.debug("repro "+ m_chromosomes.get(0)  );
			
			operator.reproduce( m_activeConfiguration, m_species, offspring, currentGeneration, maxGenerations );
		...
		while ( mutOpIter.hasNext() ) {
			...
			logger.debug( "mchrome="+ m_chromosomes.get( m_chromosomes.size() -1 )  );
			operator.mutate( m_activeConfiguration, offspring, currentGeneration, maxGenerations );
