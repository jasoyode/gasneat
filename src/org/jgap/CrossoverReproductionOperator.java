...
protected abstract ChromosomeMaterial reproduce( Configuration config,
		Chromosome dominantChrom, Chromosome recessiveChrom,  int currentGeneration, int maxGenerations );
 ...
 */
final protected void reproduce( final Configuration config, final List parentChroms,
		int numOffspring, List offspring, int currentGeneration, int maxGenerations ) {
	...
	if ( parentChroms.size() < 2 )
		...
		while ( offspring.size() < targetSize ) {
			...
			// TODO - use speciated fitness here?
			...
			ChromosomeMaterial child = reproduce( config, dominant, recessive,  currentGeneration, maxGenerations);
			offspring.add( child );
		}
	}
}
